  ...
  const addDriver = async () => {
    ...
    try {
      await addDoc(collection(db, "drivers"), {
        ...
        // Initialize with an object so the UI doesn't crash
        lastLocation: {
          address: "Awaiting GPS...",
          lat: 17.6599, // Default to Solapur HQ
          lng: 75.9064,
          speed: 0
        }
      });
            ...
            <div key={d.id} className="bg-white rounded-[2.5rem] p-5 md:p-8 shadow-sm border border-gray-100 relative hover:shadow-xl transition-shadow duration-300">
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex flex-col gap-2">
                  ...
                  <span className={`w-fit px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${isActuallyConnected ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
              ...
              <div className={`mt-6 p-5 rounded-[2rem] flex items-start gap-3 border transition-colors ${isActuallyConnected ? 'bg-blue-50 border-blue-100' : 'bg-slate-50 border-slate-100 opacity-60'
                }`}>
                <MapPin className={`${isActuallyConnected ? 'text-blue-600' : 'text-gray-400'} shrink-0 mt-1`} size={20} />
                <div className="overflow-hidden">
                  <div className="flex justify-between items-center mb-1">
                    <p className={`text-sm font-bold leading-tight ${isActuallyConnected ? 'text-blue-900' : 'text-gray-700'}`}>
                      {isActuallyConnected && d.lastLocation?.address
                        ? d.lastLocation.address
                        : !isActuallyConnected && d.lastLocation?.address && d.lastLocation.address !== "Awaiting GPS..."
                          ? `Last seen: ${d.lastLocation.address}`
